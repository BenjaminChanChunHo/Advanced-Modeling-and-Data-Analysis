...
subtitle: |
  ...
  | Full Model with Random Intercept and LRT Effect
editor_options:
...
init1 <- list(beta = c(0, 0, 0, 0, 0, 0), alpha0 = c(0, 0, 0), alpha1 = c(0, 0, 0), 
              phi0 = 1, phi1 = 1, psi = 1)
init2 <- list(beta = c(1, -1, 1, -1, -1, -1), alpha0 = c(1, -1, 1), alpha1 = c(1, -1, 1), 
              phi0 = 0.5, phi1 = 0.5, psi = 0.5)
inits <- list(init1, init2)

parameters = c("beta", "alpha0", "alpha1", "var_u0", "var_u1", "var_y")
```

### Bayesian Results
```{r}
model <- bugs(data, inits, parameters, model.file = "full_model_with_random_intercept_and_LRT_effect.txt", 
...
Variable <- c(colnames(X), rep(colnames(W), 2), "var_u0", "var_u1", "var_y")
...
rmeanplot(model, parms = "alpha1", style = "plain")

rmeanplot(model, parms = "var_u0", style = "plain")
rmeanplot(model, parms = "var_u1", style = "plain")
