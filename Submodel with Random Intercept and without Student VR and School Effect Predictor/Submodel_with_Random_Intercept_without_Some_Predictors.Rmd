...
subtitle: |
  ...
  | Submodel with Random Intercept, without VR Band and School Effect Predictor
editor_options:
...
# LR.Test.Score, Student.Gender, Student.Band.Mid, Student.Band.Top
X <- d[, c(4:5, 11:12)]
...
init1 <- list(beta = c(0, 0, 0, 0), phi0 = 1, psi = 1)
init2 <- list(beta = c(1, -1, -1, -1), phi0 = 0.5, psi = 0.5)
inits <- list(init1, init2)

parameters = c("beta", "var_u0", "var_y")
```

### Bayesian Results
```{r}
model <- bugs(data, inits, parameters, 
              model.file = "submodel_with_random_intercept_without_some_predictors.txt", 
...
Variable <- c(colnames(X), "var_u0", "var_y")
