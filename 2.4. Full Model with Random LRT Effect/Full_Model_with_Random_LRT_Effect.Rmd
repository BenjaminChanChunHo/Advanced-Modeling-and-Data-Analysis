...
subtitle: |
  ...
  | Full Model without Intercept, with Random LRT Effect
editor_options:
...
init1 <- list(beta = c(0, 0, 0, 0, 0, 0), alpha1 = c(0, 0, 0), phi1 = 1, psi = 1)
init2 <- list(beta = c(1, -1, 1, -1, -1, -1), alpha1 = c(1, -1, 1), phi1 = 0.5, psi = 0.5)
inits <- list(init1, init2)

parameters = c("beta", "alpha1", "var_u1", "var_y")
```

### Bayesian Results
```{r}
model <- bugs(data, inits, parameters, model.file = "full_model_with_random_LRT_effect.txt", 
...
Variable <- c(colnames(X), colnames(W), "var_u1", "var_y")
