...
subtitle: |
  ...
  | Full Model with Random Intercept and without Fixed Intercept
editor_options:
...
init1 <- list(beta = c(0, 0, 0, 0, 0, 0), alpha0 = c(0, 0, 0), phi0 = 1, psi = 1)
init2 <- list(beta = c(1, -1, 1, -1, -1, -1), alpha0 = c(1, -1, 1), phi0 = 0.5, psi = 0.5)
inits <- list(init1, init2)

parameters = c("beta", "alpha0", "var_u0", "var_y")
```

### Bayesian Results
```{r}
model <- bugs(data, inits, parameters, 
              model.file = "full_model_with_random_intercept_without_fixed_intercept.txt", 
...
Variable <- c(colnames(X), colnames(W), "var_u0", "var_y")
